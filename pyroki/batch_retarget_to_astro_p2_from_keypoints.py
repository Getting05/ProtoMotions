...
def get_humanoid_retarget_indices() -> jnp.ndarray:
    ...
    for human_name, astro_p2_name in [
        ...
        astro_p2_joint_retarget_indices.append(ASTRO_P2_LINK_NAMES.index(astro_p2_name))

    astro_p2_joint_retarget_indices = jnp.array(astro_p2_joint_retarget_indices)

    return human_retarget_names, astro_p2_joint_retarget_indices


human_retarget_names, astro_p2_joint_retarget_indices = None, None
...
def main():
    ...
    parser.add_argument(
        ...
        default=str(
            SCRIPT_DIR
            / "../protomotions/data/assets/astro_p2/urdf/astro_p2_retarget.urdf"
        ),
    ...
    parser.add_argument(
        "--mesh-dir",
        type=str,
        default=str(SCRIPT_DIR / "../protomotions/data/assets/astro_p2/meshes"),
    ...
    global human_retarget_names, astro_p2_joint_retarget_indices
    human_retarget_names, astro_p2_joint_retarget_indices = get_humanoid_retarget_indices()

    current_motion_index = 0

    # Create connectivity matrix for the Astro P2 joints
    n_retarget = len(astro_p2_joint_retarget_indices)
    astro_p2_retarget_mask = jnp.zeros((n_retarget, n_retarget))
    for link_a, link_b, weight in direct_pairs:
        ...
        astro_p2_retarget_mask = astro_p2_retarget_mask.at[retarget_idx_a, retarget_idx_b].set(
            weight
        )
        astro_p2_retarget_mask = astro_p2_retarget_mask.at[retarget_idx_b, retarget_idx_a].set(
            weight
        )
    ...
    if args.visualize:
        ...
        def generate_trajectory():
            ...
            Ts_world_root, joints = solve_retargeting(
                ...
                astro_p2_joint_retarget_indices=astro_p2_joint_retarget_indices,
                astro_p2_retarget_mask=astro_p2_retarget_mask,
        ...
        for i, motion_path in enumerate(test_keypoints_paths):
            ...
            results_to_save = {
                ...
                # Persist the solver order so downstream conversion can reject
                # or reorder incompatible files instead of silently assigning
                # angles to the wrong P2 joints.
                "joint_names": onp.asarray(robot.joints.actuated_names),
...
def foot_contact_cost(
    ...
    left_ankle_robot_idx = astro_p2_joint_retarget_indices[left_ankle_idx]
    right_ankle_robot_idx = astro_p2_joint_retarget_indices[right_ankle_idx]
    left_foot_robot_idx = astro_p2_joint_retarget_indices[left_foot_idx]
    right_foot_robot_idx = astro_p2_joint_retarget_indices[right_foot_idx]
...
def solve_retargeting(
    ...
    astro_p2_joint_retarget_indices: jnp.ndarray,
    astro_p2_retarget_mask: jnp.ndarray,
    ...
    # Variables.
    class SimplifiedJointsScaleVarAstroP2(
    ...
    var_joints_scale = SimplifiedJointsScaleVarAstroP2(jnp.zeros(timesteps))
    ...
    def retargeting_cost(
        ...
        var_Ts_world_root: jaxls.SE3Var,
        var_robot_cfg: jaxls.Var[jnp.ndarray],
        var_joints_scale: SimplifiedJointsScaleVarAstroP2,
    ...
    def pc_alignment_cost(
        ...
        T_root_link = jaxlie.SE3(robot.forward_kinematics(cfg=robot_cfg))
        T_world_link = T_world_root @ T_root_link
        link_pos = T_world_link.translation()[astro_p2_joint_retarget_indices]

        # hand_aux_link 1
        left_wrist = human_retarget_names.index("left_wrist")
        left_wrist_idx = astro_p2_joint_retarget_indices[left_wrist]
        ...
        # P2's forearm/wrist chain extends along local +X. This point represents
        # the palm center beyond wrist_yaw_link (the fixed hand link starts at
        # +0.0675 m and its collision center is around +0.11 m).
        ...
        right_wrist_idx = astro_p2_joint_retarget_indices[right_wrist]
